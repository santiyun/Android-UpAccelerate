...
import static com.tttrtclive.LocalConstans.CALL_BACK_ON_RTC_PUSH_STATUS;
...
public class MyTTTRtcEngineEventHandler extends TTTRtcEngineEventHandler {
    ...
    public void onRtcPushStatus(String url, boolean status) {
        MyLog.i(TAG, "onRtcPushStatus.... url ： " + url + " | status : " + status);
        JniObjs mJniObjs = new JniObjs();
        mJniObjs.mJniType = CALL_BACK_ON_RTC_PUSH_STATUS;
        mJniObjs.mUrl = url;
        mJniObjs.mPushStatus = status;
        sendMessage(mJniObjs);
    }
